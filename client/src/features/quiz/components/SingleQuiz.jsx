						...
						>
							<div className="text-center">
								...
								<div className="flex gap-4">
									...
									<button
										onClick={() => performSubmit(formData)}
										disabled={isPending}
			...
			>
				{quiz.questions.map((el, index) => (
					<motion.div
						...
						className="bg-white p-6 sm:p-10 rounded-[2rem] sm:rounded-[3rem] shadow-sm border border-gray-100 hover:shadow-md transition-all group"
					>
						<div className="flex items-start gap-4 mb-8">
							<span className="flex-shrink-0 w-10 h-10 rounded-2xl bg-indigo-50 text-indigo-600 flex items-center justify-center font-black text-lg group-hover:scale-110 transition-transform">
								{index + 1}
							</span>
						...
						<AnimatePresence>
							{errors?.answer?.[index] && (
								...
								>
									...
									<span>{errors.answer[index].message}</span>
								</motion.div>
							)}
						</AnimatePresence>
					</motion.div>
				...
				>
					...
					<button
						className="w-full sm:w-auto min-w-[300px] bg-indigo-600 hover:bg-indigo-700 text-white font-black py-5 px-12 rounded-[2rem] shadow-2xl shadow-indigo-200 transition-all hover:-translate-y-1 active:scale-95 disabled:opacity-70 disabled:cursor-not-allowed disabled:hover:translate-y-0"
